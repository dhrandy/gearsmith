...
def test_price_paid_value_and_collection_total(app_url, width, height):
    with sync_playwright() as p:
        ...
        # Settings > Features > Collection value hides the money everywhere but the edit form
        page.goto(app_url + "/#/settings")
        page.locator("[data-feature=feature_values]").uncheck()
        expect(page.get_by_text("Values hidden")).to_be_visible()
        page.goto(app_url + "/#/")
        expect(page.get_by_role("heading", name="Gear", exact=True)).to_be_visible()
        page.get_by_role("link", name=re.compile("Starling")).first.click()
        expect(page.get_by_role("button", name="Edit", exact=True)).to_be_visible()
        page.screenshot(path=f"/tmp/values-off-detail-{width}.png", full_page=True)
        expect(page.get_by_text("$1,150 (+$250)")).to_have_count(0)
        expect(page.get_by_text("$900")).to_have_count(0)
        page.get_by_role("button", name="Edit", exact=True).click()
        expect(page.locator("#gf-value")).to_have_value("1150")
        expect(page.locator("#gf-pprice")).to_have_value("900")
        page.locator("#gf-cancel").click()
        expect(page.locator("#modal")).to_be_hidden()
        page.goto(app_url + "/#/")
        expect(page.get_by_role("heading", name="Gear", exact=True)).to_be_visible()
        expect(page.get_by_text("Values hidden")).to_be_hidden(timeout=10000)
        expect(page.locator("#collection-total")).to_have_count(0)
        assert_no_overflow(page, "gear page with values off")
        page.screenshot(path=f"/tmp/values-off-gear-{width}.png")
        page.goto(app_url + "/#/want")
        expect(page.get_by_text("Dream Guitar")).to_be_visible()
        expect(page.get_by_text("3,000")).to_have_count(0)
        expect(page.locator("#collection-total")).to_have_count(0)
        page.goto(app_url + "/#/settings")
        expect(page.locator("[data-feature=feature_values]")).not_to_be_checked()
...
@pytest.mark.parametrize('width,height', [(1920, 1080), (390, 844)])
def test_token_sign_in_on_desktop_and_mobile(app_url, width, height):
    with sync_playwright() as p:
        browser = p.chromium.launch()
        page = browser.new_page(viewport={'width': width, 'height': height})
        sign_in(page, app_url)
        page.goto(app_url + '/#/settings')
        page.locator('#tk-name').fill('web sign-in')
        page.get_by_role('button', name='Create token').click()
        token = page.locator('#token-value').inner_text()
        page.get_by_role('button', name='Done').click()
        page.locator('#logout').click()
        page.reload()
        expect(page.get_by_role('button', name='API token')).to_be_visible()
        page.get_by_role('button', name='API token').click()
        expect(page.locator('#username-field')).to_be_hidden()
        expect(page.locator('#token-field')).to_be_visible()
